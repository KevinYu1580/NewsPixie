...
5. [狀態管理（Pinia Stores）](#5-狀態管理pinia-stores)
   ...
   - 5.3 [bookmarksStore](#53-bookmarksstore)
...
7. [Server API 端點](#7-server-api-端點)
   ...
   - 7.5 [POST /api/ai-models](#75-post-apiai-models)
   - 7.6 [POST /api/ai-extract-articles](#76-post-apiai-extract-articles)
   - 7.6b [POST /api/ai-curate](#76b-post-apiai-curate)
...
│   │   │   └── Card.vue     # 單篇文章卡片（含收藏按鈕）
│   │   ├── github/
│   │   │   ├── Trending.vue # GitHub Trending 渲染邏輯
│   │   │   └── RepoCard.vue # 單個 Repo 卡片（含收藏按鈕）
│   │   └── bookmarks/
│   │       └── BookmarksView.vue # 收藏清單畫面（篩選 全部/文章/Repo，見 §5.3、§8.2）
│   └── topics/
...
│   └── bookmarksStore.ts    # 收藏文章/Repo 狀態，快照式儲存（見 §5.3）
├── server/
...
│   │   └── session/         # save / clear / pubkey（見 §7.11）
│   └── utils/
│       ├── ai-client.ts     # 共用 AI client 工廠（三 provider 純 REST）+ DEFAULT_MODELS
...
├── utils/
│   └── utils.ts             # generateId、todayStr、getErrorMessage、loadTodayCache/saveTodayCache 等（auto-import）
├── types/
...
檔案：[stores/bookmarksStore.ts](stores/bookmarksStore.ts)

| State | 型別 | 說明 |
|-------|------|------|
| `articles` | `BookmarkedArticle[]` | 已收藏文章（`CuratedArticle` + `topicId`/`topicName`/`bookmarkedAt` 快照） |
| `repos` | `BookmarkedRepo[]` | 已收藏 Repo（`RepoItem` + `topicId`/`topicName`/`bookmarkedAt` 快照） |

| Computed | 說明 |
|----------|------|
| `totalCount` | `articles.length + repos.length` |

| Action | 說明 |
|--------|------|
| `isArticleBookmarked(url)` | 依 `url` 判斷文章是否已收藏 |
| `isRepoBookmarked(id)` | 依 `id` 判斷 Repo 是否已收藏 |
| `toggleArticle(article, topicId, topicName)` | 已收藏則移除、未收藏則以快照加入（含 `bookmarkedAt`） |
| `toggleRepo(repo, topicId, topicName)` | 同上，對象為 Repo |

**設計原則**：採快照式儲存而非參照式——收藏當下複製完整卡片資料，不依賴每日快取（`MAX_CACHE_DAYS = 1`，隔天即被 prune）。收藏永久保留，由使用者手動移除，不設數量上限或過期時間。

持久化：`localStorage` key `newspixie-bookmarks`，pickle `articles` + `repos`。此 key 不在 `newspixie-daily`／`newspixie-github` 的 prune 前綴範圍內（見 [§10](#10-快取策略)），不會被每日快取清理機制影響。
...
POST 端點共用 [server/utils/api-helpers.ts](server/utils/api-helpers.ts)：`readJsonBody()`（body 解析失敗統一回 400）、`parseAIJsonArray()`（AI 回覆 JSON 陣列解析含 regex fallback）、`toApiError()`（未知錯誤統一轉 createError，H3 錯誤原樣拋出）。
...
```json
{ "ok": true, "meta": { "hasKey": true, "providers": ["anthropic"], "masked": { "anthropic": "••••AB12" }, "models": { "anthropic": "claude-sonnet-4-5" }, "currentProvider": "anthropic" } }
```
...
│   └── LayoutAppSidebar
│       ├── 「我的收藏」常駐項目（bookmarksCount badge）
...
└── v-main
    ├── ContentBriefingFeed (viewMode === 'topic')
    └── ContentBookmarksView (viewMode === 'bookmarks')
```
...
`pages/index.vue` 維護 `viewMode: 'topic' | 'bookmarks'`（不持久化）決定主內容區顯示 `ContentBriefingFeed` 或 `ContentBookmarksView`；`AppSidebar` 的「我的收藏」項目 emit `selectBookmarks`、主題項目 emit `selectTopic`，兩者互斥切換 `viewMode`。
...
│   └── ContentNewsBriefing (topicId/topicName props)
│       └── ContentNewsCard × N (含收藏按鈕)
└── ContentSection (title="趨勢 Repos")
    └── ContentGithubTrending (topicId/topicName props)
        └── ContentGithubRepoCard × N (含收藏按鈕)

ContentBookmarksView（無 props，直接讀 bookmarksStore）
├── 篩選 v-btn-toggle（全部/文章/Repo）
├── ContentNewsCard × N（已收藏文章，依 bookmarkedAt 新到舊）
└── ContentGithubRepoCard × N（已收藏 Repo，依 bookmarkedAt 新到舊）
```

**BookmarksView** — 側邊欄「我的收藏」入口對應畫面，直接重用 `ContentNewsCard`／`ContentGithubRepoCard` 顯示已收藏內容，不依賴每日快取，卡片下方顯示來源主題名稱快照。收藏/取消收藏皆透過卡片右上角同一顆書籤圖示按鈕（toggle 語意）。
...
**清理機制**：兩個 composable 均透過共用的 `saveTodayCache()`／`loadTodayCache()`（[utils/utils.ts](utils/utils.ts)）讀寫快取；寫入後自動呼叫 `pruneLocalStorageCache()`，掃描所有符合 prefix 的 key，比對日期後移除超過 `MAX_CACHE_DAYS` 天的條目。
...
| [bookmarksStore](#53-bookmarksstore) | `newspixie-bookmarks` | `articles`、`repos` | ❌ 明文 |
...
> ⚠️ 目前版本庫中**沒有** `e2e/` 目錄與 `playwright.config.ts`，也沒有任何單元測試檔案；`@playwright/test`、`vitest`、`jsdom` 仍保留於 devDependencies 供未來使用。驗證程式碼品質目前依賴 `pnpm lint`（ESLint + @antfu config）。
